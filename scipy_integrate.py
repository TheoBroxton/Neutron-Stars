...
# Define parameters
initial_state = [1e-10, 2.33002e21, 0.0]  # r=1e-10 to avoid division by zero
final_time = 1.8e7  # Adjust as needed
step_size = 1e3  # Adjust as needed
...
def grad(time, state, M0, gamma_nrel, K_nrel):
    ...
    dp_dr = -(G * M0 / c**2 * p**(1/gamma_nrel) * mbar) / \
...
# Solve ODE
sol = solve_neutron_star_ode(
    initial_state, final_time, step_size, M0, gamma_nrel, K_nrel)

# Plot results
plot_neutron_star(sol)
