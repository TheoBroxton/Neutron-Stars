# -*- coding: utf-8 -*-
"""
Created on Fri Apr 19 12:26:35 2024
...
R0 = G * M0 / c**2  # Half Schwarzschild radius in km
m_N = 0.5 * (m_p + m_n)
...
def ns_calculation():
    ...
    # Plot total mass against total radius
    plot_radius_mass(total_masses_ns, total_radii_ns, filename='NS_radius_mass_plot')


def plot_radius_mass(total_masses, total_radii, filename='radius_mass_plot'):
    """
    Plot the total mass against the total radius of neutron stars.

    Parameters:
        total_masses (array-like): Array of total masses.
        total_radii (array-like): Array of total radii.
        filename (str): Name of the file to save the plot. Defaults to saving
                        in a Figures subdirectory.
    """
    plt.figure(figsize=(8, 6))
    plt.plot(total_radii, total_masses, linestyle='-')
    plt.xlabel('Total Radius (km)')
    plt.ylabel('Total Mass ($M_0$)')
    plt.title('Total Mass vs. Total Radius')
    plt.grid(True)
